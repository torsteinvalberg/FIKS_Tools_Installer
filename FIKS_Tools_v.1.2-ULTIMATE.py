...
def center_window(win):
    def _center():
        ...
        x = (sw - w) // 2
        y = (sh - h) // 2
        print(f"[DEBUG] Centering: screen={sw}x{sh}, popup={w}x{h}, pos=({x},{y})")
...
def export_to_excel_flexible(file, result, with_barcodes=False, parent_window=None, progress_func=None):
    import os
    import platform
    import pandas as pd
    ...
    if with_barcodes:
        ...
        df = generate_gtin_with_progress(df, progress_func=progress_func)
    ...
    if os.path.exists(barcode_folder):
        try:
            ...
            print("[INFO] Ryddet opp i barcode-mappen")
        except Exception as e:
            print(f"[WARN] Kunne ikke rydde barcode-mappe: {e}")
