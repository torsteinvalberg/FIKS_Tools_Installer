    ...
    def perform_extraction(self):
        ...
        if isinstance(self.extractor_instance, AdvancedShippingNoteExtractor):
            ...
            def export_to_excel_asn():
                ...
                wb = Workbook()
                ws = wb.active  # Flyttes OPP hit
                ws.title = "ASN-Export"

                from utils.barcode_utils import generate_gtin_barcodes

                all_rows = [item for group in result["Packages"].values() for item in group if item.get("GTIN")]
                df = pd.DataFrame(all_rows)
                df = generate_gtin_barcodes(df)

                print(df["StrekkodeFil"].apply(os.path.exists))  # Debug: viser hvilke filer som faktisk finnes

                barcode_map = {row["GTIN"]: row["StrekkodeFil"] for _, row in df.iterrows()}
                ...
                for sscc, items in result["Packages"].items():
                    ...
                    ws.cell(row=row_idx, column=5, value="StrekkodeFil").font = bold_font
                    row_idx += 1
                    ...
                    for item in items:
                        ...
                        if item.get("GTIN") in barcode_map:
                            ws.cell(row=row_idx, column=5, value=barcode_map[item["GTIN"]])
                ...
                # Sett inn strekkodebilder
                for row_num in range(1, ws.max_row + 1):
                    gtin = ws.cell(row=row_num, column=2).value
                    img_path = barcode_map.get(gtin)
                    if img_path and os.path.exists(img_path):
                        try:
                            ...
                            print(f"Feil ved innsetting av bilde for {gtin}: {e}")
            ...
            self.create_styled_button(btn_row, text="Eksporter til Excel", command=export_to_excel_asn).pack(side='left', padx=5)

            def export_to_excel_asn_with_barcodes():
                # Samme logikk som export_to_excel_asn, eller bruk den som base og legg til/endre ved behov.
                export_to_excel_asn()

            self.create_styled_button(btn_row, text="Eksporter til Excel med strekkoder", command=export_to_excel_asn_with_barcodes).pack(side='left', padx=5)
